"""Results analysis and visualization for diffusion models.

This module provides functions to analyze and visualize sample quality metrics
for both Time and Frequency domain diffusion models.
"""
    ...
    with plt.style.context("science"):
        for metric in df.Metric.unique():
            for dataset in df.Dataset.unique():
                ...
                if output_dir is not None:
                    filename = f"{metric.lower().replace(' ', '_')}_{dataset.lower().replace('-', '_')}.pdf"
...
def process_spectral_analysis(
    ...
    """Process spectral density analysis.
    ...
    if output_dir is not None:
        output_dir.mkdir(parents=True, exist_ok=True)
        spectral_df.to_csv(output_dir / "spectral_density.csv", index=False)
        print(f"Saved spectral density to {output_dir / 'spectral_density.csv'}")
...
def plot_spectral_density(
    ...
    """Plot spectral density analysis.
    ...
    for dataset in spectral_df.Dataset.unique():
        ...
        if output_dir is not None:
            filename = f"spectral_density_{dataset.lower().replace('-', '_')}.pdf"
            plt.savefig(output_dir / filename, bbox_inches="tight")
            print(f"Saved figure to {output_dir / filename}")
...
def main(
    ...
    """Main function to process and visualize results.
